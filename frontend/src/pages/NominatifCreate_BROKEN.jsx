  ...
  useEffect(() => {
    const fetchRKAList = async () => {
      try {
        ...
        console.log('RKA API Response Status:', response.status);

        if (response.ok) {
          const data = await response.json();
          console.log('RKA API Response:', data);
          ...
          if (Array.isArray(rkaArray)) {
            setRkaList(rkaArray);
            console.log('Successfully loaded', rkaArray.length, 'RKA items');
          } else {
            console.warn('Unexpected RKA data structure:', data);
            console.log('Available keys:', Object.keys(data));
            setRkaList([]);
          }
        } else {
          console.error('RKA API Error:', response.status, response.statusText);
          setRkaList([]);
        }
      } catch (error) {
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Pilih Code RKA untuk membuat nominatif perjalanan dinas
                </p>
              </div>
            </div>
          </div>
